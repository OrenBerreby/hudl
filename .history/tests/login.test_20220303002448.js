...
describe("Test logging into Hudl", () => {
  ...
  test.only("Successful login with correct credentials, checking remember me ticked remembers email address post logout and unticked does not.", async () => {
    ...
    await driver.get(loginUrl);
    await waitForElementAndClick(driver, "email");
    await waitForElementAndClick(driver, "email");
    await driver.findElement(By.id("email")).sendKeys(Key.BACK_SPACE);
    await login(driver, emailAddress, password);
    await logout(driver);
  ...
  test("Test all buttons on login page to check they all work", async () => {
    // at end
    await waitForElementAndClick(driver, "div.back-to-hudl");
  });
