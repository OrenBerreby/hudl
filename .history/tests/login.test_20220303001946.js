...
describe("Test logging into Hudl", () => {
  ...
  test.only("Successful login with correct credentials, checking remember me ticked remembers email address post logout and unticked does not.", async () => {
    let driver = new webdriver.Builder().forBrowser("chrome").build();

    // Login with remember me ticked
    await driver.get(loginUrl);
    await waitForAndClick(driver, ".form__label--custom");
    await login(driver, emailAddress, password);
    await logout(driver);

    // Click login to get back to the login screen
    await waitForAndClick(driver, "[data-qa-id=login]");
    ...
    await driver.get(loginUrl);
    await waitForAndClick(driver, "#email");
    await fillInputField(driver, "email", "");
    await login(driver, emailAddress, password);
    await logout(driver);
